...
async function generateXLSX(data) {
  ...
  return workbook.xlsx.writeBuffer();
    ...
    async function generatePDF(data) {
        ...
        const rowsPerPage = Math.floor((doc.page.height - 2 * margin) / cellHeight) - 1; // -1 for header row
          ...
          for (let rowIndex = startRow; rowIndex < endRow && rowIndex < tableData.length; rowIndex++) {
            ...
            row.forEach((cell, cellIndex) => {
              ...
              doc.text(
                ...
                (rowIndex - startRow + 1) * cellHeight + margin + cellPadding,
                {
                  width: cellWidth - 2 * cellPadding,
        ...
        function drawTableLines(rowCount) {
          ...
          for (let i = 0; i <= rowCount + 1; i++) {
            doc.moveTo(margin, i * cellHeight + margin)
               .lineTo(doc.page.width - margin, i * cellHeight + margin)
               .stroke();
          }
          // Vertical lines
          for (let i = 0; i <= headers.length; i++) {
            doc.moveTo(i * cellWidth + margin, margin)
               .lineTo(i * cellWidth + margin, (rowCount + 1) * cellHeight + margin)
               .stroke();
          }
        }
